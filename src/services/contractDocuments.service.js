...
export function adminApplicationNotificationHtml(application) {
  const offer = getPackageOffer(application.package_key);
  return documentShell('Neuer PDB PREMIUM Vertragsantrag', `<h1>Neuer PDB PREMIUM Vertragsantrag</h1><p>Ein neuer Antrag wurde sicher gespeichert und wartet auf die SEPA-Einrichtung sowie die ausdrückliche Annahme.</p><table><tr><td>Name</td><td>${escapeHtml(application.first_name)} ${escapeHtml(application.last_name)}</td></tr><tr><td>E-Mail</td><td>${escapeHtml(application.email)}</td></tr><tr><td>Paket</td><td>${escapeHtml(offer?.name)}</td></tr><tr><td>Monatsbeitrag</td><td>${money(application.monthly_price_cents)}</td></tr><tr><td>Vertragsbeginn</td><td>${escapeHtml(application.starts_on)}</td></tr><tr><td>Mandatsreferenz</td><td>${escapeHtml(application.mandate_reference)}</td></tr><tr><td>IBAN</td><td>${escapeHtml(maskIban(application.iban_last4))}</td></tr><tr><td>Status</td><td>SEPA-Einrichtung ausstehend</td></tr></table><div class="note"><strong>Sicherheit:</strong> Die vollständige IBAN befindet sich nicht in dieser E-Mail. Öffnen Sie die geschützte Vertragsverwaltung unter https://pdb-premium-system.onrender.com/admin/contracts und verwenden Sie dort den ADMIN_API_TOKEN.</div>`);
}
